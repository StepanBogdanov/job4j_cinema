...
public class Sql2oTicketRepository implements TicketRepository {
    ...
    public Optional<Ticket> save(Ticket ticket) {
        try (var connection = sql2o.open()) {
            var sql = """
                    INSERT INTO tickets (session_id, row_number, place_number, user_id) values (:sessionId, :rowNumber, :placeNumber, :userId)
                    """;
    ...
    @Override
    public Optional<Ticket> findByUniqueParameters(int sessionId, int rowNumber, int placeNumber) {
        try (var connection = sql2o.open()) {
            var query = connection.createQuery(
                    "SELECT * FROM tickets WHERE session_id = :sessionId AND row_number = :rowNumber AND place_number = :placeNumber");
            var ticket = query.addParameter("sessionId", sessionId)
